...
class Projetil:
    ...
    def colidir(self, alvo):
        """
        Verifica colisão com um lutador.
        Usa geometria de círculo: dist < (raio_projétil + raio_alvo)
        
        BUG-FIREBALL-FIX v1.0: Implementa método colidir que estava faltando.
        Sem este método, Projetil cai no fallback que era menos preciso.
        """
        if alvo == self.dono:
            return False
        if alvo.morto:
