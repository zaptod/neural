...
def load_test_characters(chars_file: str) -> List[Dict]:
    ...
    if not os.path.exists(chars_file):
        print(f"❌ File not found: {chars_file}")
        return []
...
def run_stress_test(
    ...
    for i in range(num_battles):
        ...
        if result:
            ...
            print(f"✓ {winner_name} ({result.duration_frames}f)")
        else:
            failed += 1
            print(f"✗")
...
if __name__ == "__main__":
    # Load characters
    chars_file = "stress_test_data/test_characters.json"
    characters = load_test_characters(chars_file)
    
    if not characters:
        print("❌ No characters loaded. Run generate_stress_test_characters.py first.")
        sys.exit(1)
