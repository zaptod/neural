"""diagnostico_hitbox.py — Diagnóstico v2.0 (geometria removida)."""
from core.hitbox import HITBOX_PROFILES, get_hitbox_profile
...
def diagnosticar_arma_hitbox(arma_dict):
    ...
    if not get_hitbox_profile(tipo):
        problemas.append({"severidade": "AVISO", "problema": f"Tipo '{tipo}' sem perfil dedicado",
                           "sugestao": f"Tipos válidos: {list(HITBOX_PROFILES.keys())}"})
...
def imprimir_relatorio(armas):
    ...
    if not res: print("✓ Todas as armas OK"); return
    for r in res:
        print(f"\n[{r['nome']}]")
        for p in r["problemas"]:
            print(f"  {p['severidade']}: {p['problema']} → {p['sugestao']}")


def obter_info_tipo(tipo):
    p = get_hitbox_profile(tipo)
    return {"tipo": tipo, "range_mult": p.get("range_mult", 2.0),
            "nota": "Tamanho depende do personagem (raio_fisico × range_mult)"}
