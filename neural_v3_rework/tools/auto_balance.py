"""
NEURAL FIGHTS — Auto Balance Tournament  [F02]
===============================================
Roda um torneio round-robin headless e imprime win-rates por personagem,
classe e arma. Usado para medir o efeito de mudanças em balance_config.py.

Uso:
    python tools/auto_balance.py                  # 50 lutas por matchup
    python tools/auto_balance.py --fights 200     # 200 lutas por matchup
    python tools/auto_balance.py --top 20         # mostrar top 20
    python tools/auto_balance.py --sample 8       # 8 personagens aleatórios

Fluxo:
    1. Carrega personagens do AppState.
    2. Gera todos os matchups únicos (round-robin).
    3. Para cada matchup, roda N lutas headless (sem pygame window).
    4. Agrega win-rates e imprime relatório.
...
def _print_report(
    ...
    """Imprime relatório completo de win-rates."""
    ...
    print()
    print("=" * 70)
    print(f"  AUTO BALANCE REPORT — {total_fights} lutas em {elapsed:.1f}s")
    print("=" * 70)
    ...
    # ── Matchups desequilibrados (WR > 70%) ───────────────────────────────────
    problematic = [ms for ms in matchups if ms.total >= 10 and (ms.wr_a > 0.70 or ms.wr_b > 0.70)]
    if problematic:
        print(f"\n⚠  MATCHUPS DESEQUILIBRADOS (WR > 70% em {len(problematic)} matchups):")
...
def run(fights: int = DEFAULT_FIGHTS, top: int = DEFAULT_TOP, sample: int = DEFAULT_SAMPLE) -> None:
    ...
    if not chars:
        print("❌  Nenhum personagem no AppState. Rode o gerador de database primeiro.")
        return

    if sample > 0 and sample < len(chars):
        chars = random.sample(chars, sample)

    print(f"▶  {len(chars)} personagens | {fights} luta(s)/matchup | headless")
    ...
    print(f"▶  {len(pairs)} matchups × {fights} = {total} lutas totais\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=f"Lutas por matchup (padrão: {DEFAULT_FIGHTS})"
    )
    parser.add_argument(
        "--top", type=int, default=DEFAULT_TOP,
        help=f"Mostrar top N personagens (padrão: {DEFAULT_TOP})"
    )
    parser.add_argument(
        "--sample", type=int, default=DEFAULT_SAMPLE,
        help="Usar N personagens aleatórios (0 = todos)"
    )
