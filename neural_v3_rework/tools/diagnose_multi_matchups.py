...
def print_matchup_result(result: MatchupResult) -> None:
    ...
    if p1_passive:
        print(f"  ⚠️  P1 shows PASSIVE behavior (advance: {result.p1_advance_pct:.1%}, block: {result.p1_block_pct:.1%})")
    else:
        print(f"  ✓ P1 behavior OK")
    
    if p2_passive:
        print(f"  ⚠️  P2 shows PASSIVE behavior (advance: {result.p2_advance_pct:.1%}, block: {result.p2_block_pct:.1%})")
    else:
        print(f"  ✓ P2 behavior OK")
...
def run_comprehensive_tests():
    ...
    for result in results:
        if result.is_passive(1):
            passive_issues.append(
                f"  • {result.p1_name} ({result.p1_weapon}) vs {result.p2_name} ({result.p2_weapon}): "
                f"Too much blocking ({result.p1_block_pct:.1%}), not enough advancing ({result.p1_advance_pct:.1%})"
            )
        if result.is_passive(2):
            passive_issues.append(
                f"  • {result.p2_name} ({result.p2_weapon}) vs {result.p1_name} ({result.p1_weapon}): "
    ...
    if passive_issues:
        ...
        print("\n✓ No significant passive behavior detected!")
