"""
NEURAL FIGHTS â€” nucleo/lutador/combat_mixin.py  [D03/D04 Sprint 8]
=================================================================
CombatMixin: dano, status effects, buffs/DoTs, morte.

ContÃ©m tambÃ©m StatusSnapshot (movida de entities.py para aqui para
quebrar a dependÃªncia de importaÃ§Ã£o circular).

Imports inline eliminados (D04): DotEffect, ENCANTAMENTOS e todos os
helpers de balance_config ficam no topo do mÃ³dulo.
"""
...
# ---------------------------------------------------------------------------
# A05: classe leve que substitui os objetos anÃ´nimos criados via type()
# ---------------------------------------------------------------------------
class StatusSnapshot:
    """Snapshot imutÃ¡vel de um status effect para leitura pela IA e magic_system."""
    __slots__ = (
...
# ---------------------------------------------------------------------------
class CombatMixin:
    """Mixin com toda a lÃ³gica de combate de Lutador."""
    ...
    def calcular_dano_ataque(self, dano_base):
        """Calcula dano final com crÃ­tico, encantamentos e PASSIVA da arma."""
        ...
        # v15.0 â€” BÃ´nus por mecÃ¢nica de arma
    ...
    def aplicar_passiva_em_hit(self, dano_aplicado, alvo, pos_impacto_px=None):
        ...
        # v15.0 â€” Efeitos on-hit por tipo de arma
        tipo_arma = getattr(self.dados, 'tipo_arma', '')
    ...
    def _atualizar_dots(self, dt):
        ...
        # A05: usa StatusSnapshot em vez de type() anÃ´nimo
        self.status_effects = [
        ...
        # D02: cc_effects â€” lista observÃ¡vel dos CCs ativos
        cc = []
        if self.stun_timer > 0:
    ...
    def tomar_dano(self, dano, empurrao_x, empurrao_y, tipo_efeito="NORMAL", atacante=None):
        """Recebe dano com suporte a efeitos e reflexÃ£o."""
    ...
    def aplicar_cc(self, efeito: str, duracao: float = None, intensidade: float = 1.0) -> None:
        """
        D02: Ponto de entrada PÃšBLICO para aplicar qualquer CC ou status.
    ...
    def _aplicar_efeito_status(self, efeito, duracao=None, intensidade=1.0):
        """Aplica efeitos de status do dano â€” Sistema v2.0 COLOSSAL."""
        # DoTs
    ...
    def morrer(self):
        ...
        # CM-14: FP1 â€” verifica buffs com ativa_ao_morrer
