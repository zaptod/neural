...
Define os limites do mapa, obstÃ¡culos e colisÃµes.
Sistema expandido com mÃºltiplos mapas temÃ¡ticos.
"""

import math
import time
import pygame
from utilitarios.config import PPM, LARGURA, ALTURA
...
ARENAS = {
    ...
    "Arena": ArenaConfig(
        ...
        formato="retangular",
        tema="classico",
        descricao="Arena padrÃ£o sem obstÃ¡culos",
        icone="ðŸŸï¸",
    ),
    ...
    "Arena Pequena": ArenaConfig(
        ...
        formato="retangular",
        tema="classico",
        descricao="EspaÃ§o apertado - combate intenso",
        icone="ðŸ“¦",
    ),
    ...
    "Coliseu": ArenaConfig(
        ...
        icone="ðŸ›ï¸",
        obstaculos=[
            # Pilares mais prÃ³ximos do centro para serem visÃ­veis
    ...
    "Floresta": ArenaConfig(
        ...
        formato="retangular",
        tema="floresta",
        descricao="Ãrvores bloqueiam a visÃ£o",
        icone="ðŸŒ²",
    ...
    "Caverna": ArenaConfig(
        ...
        formato="retangular",
        tema="caverna",
        descricao="Cristais brilham na escuridÃ£o",
        icone="ðŸ’Ž",
    ...
    "Castelo": ArenaConfig(
        ...
        formato="retangular",
        tema="medieval",
        descricao="O salÃ£o real aguarda sangue",
        icone="ðŸ°",
        obstaculos=[
            # Pilares do salÃ£o
    ...
    "Espacial": ArenaConfig(
        ...
        formato="retangular",
        tema="scifi",
        descricao="Gravidade artificial instÃ¡vel",
        icone="ðŸš€",
    ...
    "Templo": ArenaConfig(
        ...
        formato="octogono",
        tema="ruinas",
        descricao="RuÃ­nas de poder antigo",
        icone="ðŸ—¿",
        obstaculos=[
            # Pilares quebrados
    ...
    "Gelo": ArenaConfig(
        ...
        formato="retangular",
        tema="gelo",
        descricao="SuperfÃ­cie escorregadia",
        icone="â„ï¸",
    ...
    "Labirinto": ArenaConfig(
        ...
        formato="retangular",
        tema="labirinto",
        descricao="Muitos obstÃ¡culos para usar",
        icone="ðŸ§±",
        obstaculos=[
    ...
    "Campo de Batalha": ArenaConfig(
        ...
        formato="retangular",
        tema="batalha",
        descricao="Arena enorme para batalhas em equipe (atÃ© 4v4)",
        icone="âš”ï¸",
        obstaculos=[
            # Barricadas centrais
    ...
    "Grande Coliseu": ArenaConfig(
        ...
        formato="circular",
        tema="romano",
        descricao="Coliseu grandioso para batalhas Ã©picas em equipe",
        icone="ðŸŸï¸",
        obstaculos=[
            # Pilares internos em cÃ­rculo
    ...
    "Fortaleza": ArenaConfig(
        ...
        formato="retangular",
        tema="ruinas",
        descricao="RuÃ­nas de uma fortaleza - muita cobertura",
        icone="ðŸšï¸",
        obstaculos=[
...
# Lista ordenada de mapas para seleÃ§Ã£o
LISTA_MAPAS = [
...
def get_mapa_info(nome: str) -> dict:
    """Retorna informaÃ§Ãµes do mapa para exibiÃ§Ã£o"""
...
class Arena:
    """
    Sistema de arena com limites, paredes e obstÃ¡culos.
    """
    ...
    def __init__(self, config: ArenaConfig = None):
        ...
        # HistÃ³rico de colisÃµes para efeitos
        self.colisoes_recentes: List[Tuple[float, float, float]] = []  # (x, y, intensidade)
        
        # Cooldown de som de colisÃ£o por lutador (evita som repetido ao deslizar)
    ...
    def danificar_obstaculo(self, obs: "Obstaculo", dano: float) -> bool:
        """
        C04: Aplica dano a um obstÃ¡culo destruÃ­vel.
        Retorna True se o obstÃ¡culo foi destruÃ­do neste hit.
        ObstÃ¡culos destruÃ­dos ficam com solido=False (atravessÃ¡veis).
        """
        ...
        if obs.hp <= 0:
            obs.hp = 0
            obs.solido = False  # Torna atravessÃ¡vel â€” remove da colisÃ£o
            return True         # DestruÃ­do
        return False

    def colide_obstaculo(self, x: float, y: float, raio: float) -> Optional[Obstaculo]:
        """
        Verifica se uma posiÃ§Ã£o colide com algum obstÃ¡culo sÃ³lido.
        Retorna o obstÃ¡culo colidido ou None.
        """
        for obs in self.obstaculos:
            if not obs.solido:
                continue
            
            # AABB collision com cÃ­rculo
            half_w = obs.largura / 2
            half_h = obs.altura / 2
            
            # Ponto mais prÃ³ximo no retÃ¢ngulo
            closest_x = max(obs.x - half_w, min(x, obs.x + half_w))
            closest_y = max(obs.y - half_h, min(y, obs.y + half_h))
            
            # DistÃ¢ncia ao ponto mais prÃ³ximo
    ...
    def esta_em_zona_perigo(self, x: float, y: float) -> Optional[str]:
        """
        Verifica se estÃ¡ em zona de perigo (lava, fogo, etc.)
    ...
    def aplicar_limites(self, lutador, dt: float) -> float:
        ...
        Retorna a intensidade do impacto (0.0 se nÃ£o houve impacto significativo).
        ...
        if self.config.formato == "circular":
            ...
            if dist + margem > self.raio:
                ...
                # Calcula velocidade perpendicular Ã  parede ANTES do rebote
                ...
                # Rebote apenas se indo em direÃ§Ã£o Ã  parede
                if dot > 0:
            ...
            # Arena octogonal (simplificado como retÃ¢ngulo com cantos cortados)
            ...
            for cx, cy, dx, dy in cantos:
                ...
                if in_x and in_y:
                    # EstÃ¡ no canto - aplica limite diagonal
            ...
            # Arena retangular padrÃ£o
            impacto_ret = self._aplicar_limites_retangular(lutador, margem, bounce)
            impacto_max = max(impacto_max, impacto_ret)
        
        # Aplica colisÃ£o com obstÃ¡culos
    ...
    def _aplicar_colisao_obstaculos(self, lutador, margem: float, bounce: float) -> float:
        """Aplica colisÃ£o com obstÃ¡culos sÃ³lidos. Retorna intensidade do impacto."""
        impacto_max = 0.0
        ...
        for obs in self.obstaculos:
            ...
            if left < px < right and top < py < bottom:
                ...
                # Registra colisÃ£o apenas se impacto significativo
                if impacto_max > 2:
    ...
    def _aplicar_limites_retangular(self, lutador, margem: float, bounce: float) -> float:
        ...
        if lutador.pos[0] - margem < self.min_x:
            ...
            # SÃ³ conta como impacto se estava indo em direÃ§Ã£o Ã  parede
            if lutador.vel[0] < 0:
    ...
    def esta_dentro(self, x: float, y: float, margem: float = 0) -> bool:
        """Verifica se um ponto estÃ¡ dentro da arena"""
        if self.config.formato == "circular":
            dx = x - self.centro_x
            dy = y - self.centro_y
    ...
    def get_spawn_points_multi(self, num_fighters: int, teams: dict) -> List[Tuple[float, float]]:
        ...
        Args:
            num_fighters: NÃºmero total de lutadores
    ...
    def _clamp_to_arena(self, x: float, y: float, margem: float = 2.0) -> Tuple[float, float]:
        """Garante que o ponto estÃ¡ dentro da arena com margem de seguranÃ§a."""
        x = max(self.min_x + margem, min(x, self.max_x - margem))
        y = max(self.min_y + margem, min(y, self.max_y - margem))
        return (x, y)

    def limpar_colisoes(self):
        """Limpa histÃ³rico de colisÃµes apÃ³s processamento"""
        # Limpa completamente - colisÃµes jÃ¡ foram processadas neste frame
    ...
    def _desenhar_obstaculos(self, surface: pygame.Surface, camera):
        """Desenha os obstÃ¡culos da arena"""
        for obs in self.obstaculos:
            ...
            if obs.tipo in ["pilar", "pilar_quebrado"]:
                ...
                # Tapete decorativo (nÃ£o sÃ³lido)
                pygame.draw.rect(surface, cor, rect)
                # Bordas douradas
    ...
    def _desenhar_paredes(self, surface: pygame.Surface, camera):
        ...
        if self.config.formato == "circular":
            # Borda circular - sÃ³ desenha o ANEL da parede, nÃ£o o interior
            cx, cy = camera.converter(self.centro_x * PPM, self.centro_y * PPM)
    ...
    def _desenhar_efeitos_colisao(self, surface: pygame.Surface, camera):
        """Desenha efeitos visuais de colisÃ£o com paredes"""
        for i, (x, y, intensidade) in enumerate(self.colisoes_recentes):
            # Fade baseado na posiÃ§Ã£o na lista (mais recente = mais visÃ­vel)
...
# InstÃ¢ncia global da arena (pode ser substituÃ­da)
_arena_atual: Optional[Arena] = None

def get_arena() -> Arena:
    """ObtÃ©m a arena atual"""
    global _arena_atual
