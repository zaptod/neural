...
================================================================================
Reescrito com foco em PRESENÃ‡A VISUAL por elemento.
Cada elemento tem geometria, ritmo e personalidade Ãºnicos:

  FOGO    â€” ExplosÃµes de calor em cascata, brasas que sobem, colunas de chamas
  GELO    â€” Cristais que se formam devagar, explosÃ£o de estilhaÃ§os, gelo que racha
  RAIO    â€” Arcos elÃ©tricos em galhos, overcharge flash cegante, chain lightning
  TREVAS  â€” VÃ³rtex que aspira luz, tentÃ¡culos de sombra, distorÃ§Ã£o de espaÃ§o
  LUZ     â€” Raios divinos, halo pulsante, explosÃ£o de judgnment com remanÃªncia
  NATUREZA â€” Espinhos que brotam do chÃ£o, esporos, raÃ­zes que prendem
  ARCANO  â€” Runas gravitando, ondas arcanas, fragmentos de Ã©ter
  CAOS    â€” Cores impossÃ­veis, geometria quebrada, efeitos misturados
  SANGUE  â€” Gotas que espirram, nÃ©voa carmesim, cÃ­rculos rituais
  VOID    â€” AusÃªncia de luz, distorÃ§Ã£o visual, buraco negro
"""
...
ELEMENT_PALETTES = {
    ...
    # B6 fix: paletas ausentes para TEMPO e GRAVITACAO
    "TEMPO": {
        "core":   (220, 210, 255),
        "mid":    [(190, 170, 255), (160, 140, 240), (130, 110, 220)],
        "outer":  [(100, 80, 200), (75, 55, 170), (50, 35, 140)],
        "spark":  (240, 235, 255),
        "glow":   (170, 150, 255, 110),
    },
    "GRAVITACAO": {
        "core":   (160, 100, 220),
        "mid":    [(110, 50, 180),  (80, 25, 150),  (55, 10, 120)],
        "outer":  [(40, 0, 95),    (25, 0, 70),    (15, 0, 50)],
        "spark":  (200, 160, 255),
        "glow":   (100, 40, 180, 100),
    },
    "DEFAULT": {
...
class DramaticProjectileTrail:
    ...
    def update(self, dt, x, y, velocidade=1.0):
        ...
        max_spawns_por_frame = 20
        spawns_feitos = 0
        while self.spawn_timer > rate and spawns_feitos < max_spawns_por_frame:
            self.spawn_timer -= rate
            self._spawn(x, y)
            spawns_feitos += 1
        if spawns_feitos >= max_spawns_por_frame:
            # Evita ciclo de recuperaÃ§Ã£o infinito quando o frame atrasa demais.
            self.spawn_timer = min(self.spawn_timer, rate)
        self.particulas = [p for p in self.particulas if p.update(dt)]
        max_particulas_trail = 500
        if len(self.particulas) > max_particulas_trail:
            self.particulas = self.particulas[-max_particulas_trail:]
    ...
    def _spawn(self, x, y):
        ...
        if el == "FOGO":
            ...
            # v14.0: Trail temporal â€” partÃ­culas que "congelam" no ar e desvanecem
            cor = random.choice(pal["mid"])
            self.particulas.append(MagicParticle(
            ...
            # Eco temporal (partÃ­cula ghost atrasada)
            ...
            # v14.0: Trail gravitacional â€” partÃ­culas que orbitam o projÃ©til
            cor = random.choice(pal["mid"])
            ...
            # v14.0: Trail caÃ³tico â€” formas e cores aleatÃ³rias
            ...
            # v14.0: Trail void â€” partÃ­culas que implosionam para o centro
            cor = random.choice(pal["mid"])
            ang = random.uniform(0, math.pi * 2)
...
class DramaticExplosion:
    def __init__(self, x, y, elemento="DEFAULT", tamanho=1.0, dano=0):
        ...
        self.ground_crack_alpha = 0  # Para efeitos de chÃ£o
        self._spawn(tamanho, dano)

    def _spawn(self, tam, dano):
        el = self.elemento
        pal = self.palette
        # Ondas de choque base (todas as explosÃµes)
        for i in range(4):
        ...
        if el == "FOGO":
            ...
            # Cristais que ficam no chÃ£o (8 direÃ§Ãµes)
            for i in range(8):
            ...
            self.flash_alpha = 255
            # PartÃ­culas elÃ©tricas rÃ¡pidas
            for _ in range(n):
                ang = random.uniform(0, math.pi * 2)
            ...
            # v14.0: Buraco negro central sugando partÃ­culas
            ...
            # DistorÃ§Ã£o visual â€” anel externo escuro
            self.shockwaves.append({
            ...
            # v14.0: ExplosÃ£o temporal â€” relÃ³gios, distorÃ§Ã£o, ecos
            self.vida = self.vida_max = 1.8
            self.flash_raio = 40 * tam
            self.flash_alpha = 200
            # PartÃ­culas temporais que se movem em espiral
            for _ in range(n):
                ang = random.uniform(0, math.pi * 2)
                vel = random.uniform(60, 200) * tam
                cor = random.choice(pal["mid"])
            ...
            # Ecos temporais â€” anÃ©is que CONTRAEM (tempo revertendo)
            for i in range(5):
                self.shockwaves.append({
            ...
            # RelÃ³gio fantasma â€” cristais em 12 posiÃ§Ãµes como marcadores de horas
            for i in range(12):
                ang = i * (math.pi * 2 / 12)
                dist = 45 * tam
                self.crystals.append({
            ...
            # Ponteiros do relÃ³gio (2 pilares giratÃ³rios)
            for i in range(2):
                self.pillars.append({
                    "x": self.x, "y": self.y,
            ...
            # v14.0: ExplosÃ£o gravitacional â€” implosÃ£o seguida de explosÃ£o
            self.vida = self.vida_max = 1.6
            self.flash_raio = 50 * tam
            self.flash_alpha = 180
            # Fase 1: PartÃ­culas puxadas para dentro
            ...
            # Fase 2: PartÃ­culas que explodem para fora (depois de colapsar)
            ...
            # VÃ³rtex gravitacional â€” espiral achatada
            for i in range(3):
                self.vortex_rings.append({
                    "raio": (20 + i * 25) * tam,
            ...
            # v14.0: ExplosÃ£o caÃ³tica â€” cores impossÃ­veis, geometria quebrada
            self.vida = self.vida_max = 1.3
            self.flash_raio = 45 * tam
            self.flash_alpha = 255
            # PartÃ­culas multicoloridas com comportamento errÃ¡tico
            for _ in range(int(n * 1.3)):
                ang = random.uniform(0, math.pi * 2)
                vel = random.uniform(100, 400) * tam
                # Cores totalmente aleatÃ³rias
            ...
            # Raios caÃ³ticos em direÃ§Ãµes aleatÃ³rias
            for i in range(4):
                ang = random.uniform(0, math.pi * 2)
            ...
            # Cristais de caos â€” fragmentos de realidade
            for i in range(6):
                ang = random.uniform(0, math.pi * 2)
    ...
    def draw(self, tela, cam):
        ...
        for p in self.pillars:
            ...
            if p.get("tem_offset", False):
                # Pilar com origem prÃ³pria em pixels
...
class DramaticAura:
    def __init__(self, x, y, raio, elemento="DEFAULT", intensidade=1.0):
        ...
        # Raios de energia periÃ³dicos (RAIO / LUZ)
        self.energy_lines = []
...
class MagicCastChargeup:
    def __init__(self, x, y, elemento="DEFAULT", duracao=0.6, intensidade=1.0):
        ...
        self.rings = []
        # PrÃ©-gera alguns anÃ©is de carga
        for i in range(3):
            self.rings.append({
    ...
    def update(self, dt, x=None, y=None):
        ...
        if self.vida <= 0:
            return False
        self.timer += dt
        prog = 1 - self.vida / self.vida_max  # 0â†’1 ao longo do chargeup
        self.anel_raio = max(4, (70 * self.intensidade) * (1 - prog * 0.75))
        for ring in self.rings:
            ring["fase"] += ring["vel"] * dt
        # PartÃ­culas aspiradas para o centro (quantidade aumenta com prog)
    ...
    def _spawn_particle(self, prog):
        ...
        px = self.x + math.cos(ang) * dist
        py = self.y + math.sin(ang) * dist
        # PartÃ­cula voa em direÃ§Ã£o ao centro com velocidade normalizada
        dx = self.x - px
        dy = self.y - py
        d = math.hypot(dx, dy)
...
class MagicVFXManager:
    ...
    def update(self, dt):
        ...
        # BUG FIX: AvanÃ§ar partÃ­culas residuais nos trails para que desapareÃ§am naturalmente.
        # Trails Ã³rfÃ£os (projÃ©til morreu) nÃ£o recebem update() via simulacao,
        # entÃ£o suas partÃ­culas ficavam congeladas no espaÃ§o indefinidamente.
