...
class WeaponTrailEnhanced:
    ...
    def draw(self, tela, cam, ppm: float):
        ...
        for i in range(1, len(self.pontos)):
            ...
            min_x = min(sx1, sx2) - largura - 6
            min_y = min(sy1, sy2) - largura - 6
            max_x = max(sx1, sx2) + largura + 6
            max_y = max(sy1, sy2) + largura + 6
            surf_w = max(8, int(max_x - min_x))
            surf_h = max(8, int(max_y - min_y))
            surf = pygame.Surface((surf_w, surf_h), pygame.SRCALPHA)
            p1_local = (int(sx1 - min_x), int(sy1 - min_y))
            p2_local = (int(sx2 - min_x), int(sy2 - min_y))
            
            # Glow se configurado
            if brilho and largura > 2:
                glow_cor = (min(255, cor[0] + 50), min(255, cor[1] + 50), min(255, cor[2] + 50), max(0, min(255, alpha // 3)))
                pygame.draw.line(surf, glow_cor, p1_local, p2_local, largura + 4)
            
            # Trail principal
            pygame.draw.line(surf, (*cor, alpha), p1_local, p2_local, largura)
            
            # Core brilhante para trails largos
            if largura > 4:
                pygame.draw.line(surf, (255, 255, 255, max(0, min(255, int(alpha * 0.75)))), p1_local, p2_local, max(1, largura // 3))

            tela.blit(surf, (int(min_x), int(min_y)))
...
class AttackAnticipation:
    ...
    def draw(self, tela, cam, ppm: float):
        ...
        for linha in self.linhas:
            ...
            surf_h = max(8, int(max_y - min_y))
            surf = pygame.Surface((surf_w, surf_h), pygame.SRCALPHA)
            p1_local = (int(ex - min_x), int(ey - min_y))
            p2_local = (int(sx - min_x), int(sy - min_y))
            pygame.draw.line(surf, cor, p1_local, p2_local, 2)
            tela.blit(surf, (int(min_x), int(min_y)))
