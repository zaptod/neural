...
Texto flutuante e manchas (decals) com animaÃ§Ãµes melhoradas
"""

import pygame
import math
from utilitarios.config import PRETO


class FloatingText:
    """Texto flutuante para dano e notificaÃ§Ãµes â€” v15.0 com scale + bounce"""
    def __init__(self, x, y, texto, cor, tamanho=20):
        ...
        self.vel_y = -1.5  # Sobe mais rÃ¡pido
        self.vel_x = 0      # Drift lateral sutil
        self.vida = 1.2
        self.max_vida = 1.2
        self.alpha = 255
        self.scale = 0.3    # ComeÃ§a pequeno (pop-in)
        
        # Texto grande (dano alto, CRITICAL, FATAL) = efeito maior
        self._is_special = tamanho >= 28 or any(w in str(texto) for w in ["!", "FATAL", "CRÃTICO", "CLASH", "PARRY"])
        if self._is_special:
            self.vel_y = -2.0
    ...
    def update(self, dt):
        ...
        # Scale animation: pop-in rÃ¡pido, depois estabiliza
...
class Decal:
    """Manchas no chÃ£o (sangue, queimaduras, etc)"""
