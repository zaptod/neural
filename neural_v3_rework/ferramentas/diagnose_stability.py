...
from ferramentas.diagnose_multi_matchups import run_matchup_diagnostic, MatchupResult
...
def run_stability_tests(
    ...
    for run in range(num_runs):
        ...
        try:
            ...
            print(f"âœ“ (avg_dist={result.p1_avg_dist:.2f}m, adv={result.p1_advance_pct:.1%})")
...
def print_stability_report(p1_name: str, p2_name: str, results: list, stats: dict) -> None:
    ...
    # P1 Report
    print(f"\n[P1] {p1_name}")
    print(f"Advance:  Î¼={stats['p1_advance']['mean']:.1%}  Ïƒ={stats['p1_advance']['std']:.3f}  range=[{stats['p1_advance']['min']:.1%}, {stats['p1_advance']['max']:.1%}]")
    print(f"Retreat:  Î¼={stats['p1_retreat']['mean']:.1%}  range=[{stats['p1_retreat']['min']:.1%}, {stats['p1_retreat']['max']:.1%}]")
    print(f"Block:    Î¼={stats['p1_block']['mean']:.1%}  range=[{stats['p1_block']['min']:.1%}, {stats['p1_block']['max']:.1%}]")
    print(f"Distance: Î¼={stats['p1_dist']['mean']:.2f}m range=[{stats['p1_dist']['min']:.2f}m, {stats['p1_dist']['max']:.2f}m]")
    
    # P2 Report
    print(f"\n[P2] {p2_name}")
    print(f"Advance:  Î¼={stats['p2_advance']['mean']:.1%}  Ïƒ={stats['p2_advance']['std']:.3f}  range=[{stats['p2_advance']['min']:.1%}, {stats['p2_advance']['max']:.1%}]")
    print(f"Retreat:  Î¼={stats['p2_retreat']['mean']:.1%}  range=[{stats['p2_retreat']['min']:.1%}, {stats['p2_retreat']['max']:.1%}]")
    print(f"Block:    Î¼={stats['p2_block']['mean']:.1%}  range=[{stats['p2_block']['min']:.1%}, {stats['p2_block']['max']:.1%}]")
    print(f"Distance: Î¼={stats['p2_dist']['mean']:.2f}m range=[{stats['p2_dist']['min']:.2f}m, {stats['p2_dist']['max']:.2f}m]")
    ...
    if p1_advance_variation < 0.05:
        print(f"âœ“ Very Consistent (Ïƒ={p1_advance_variation:.3f})")
    elif p1_advance_variation < 0.10:
        print(f"âœ“ Consistent (Ïƒ={p1_advance_variation:.3f})")
    else:
        print(f"âš ï¸  Variable (Ïƒ={p1_advance_variation:.3f})")
    
    print(f"P2 consistency: ", end="")
    if p2_advance_variation < 0.05:
        print(f"âœ“ Very Consistent (Ïƒ={p2_advance_variation:.3f})")
    elif p2_advance_variation < 0.10:
        print(f"âœ“ Consistent (Ïƒ={p2_advance_variation:.3f})")
    else:
        print(f"âš ï¸  Variable (Ïƒ={p2_advance_variation:.3f})")
    ...
    if p1_mean_adv > 0.50 and p1_mean_blk < 0.10:
        print(f"P1: âœ“ Aggressive & Engaged (advance={p1_mean_adv:.1%}, block={p1_mean_blk:.1%})")
    elif p1_mean_adv > 0.40 and p1_mean_blk < 0.15:
        print(f"P1: âœ“ Balanced (advance={p1_mean_adv:.1%}, block={p1_mean_blk:.1%})")
    else:
        print(f"P1: âš ï¸  Passive (advance={p1_mean_adv:.1%}, block={p1_mean_blk:.1%})")
    
    if p2_mean_adv > 0.50 and p2_mean_blk < 0.10:
        print(f"P2: âœ“ Aggressive & Engaged (advance={p2_mean_adv:.1%}, block={p2_mean_blk:.1%})")
    elif p2_mean_adv > 0.40 and p2_mean_blk < 0.15:
        print(f"P2: âœ“ Balanced (advance={p2_mean_adv:.1%}, block={p2_mean_blk:.1%})")
    else:
        print(f"P2: âš ï¸  Passive (advance={p2_mean_adv:.1%}, block={p2_mean_blk:.1%})")
...
def main():
    ...
    if issues_found:
        ...
        print("\nâœ“ All matchups show healthy behavioral patterns!")
        print("  â€¢ Melee aggressiveness: Appropriate")
        print("  â€¢ Ranged behavior: Appropriately cautious")
        print("  â€¢ Consistency across seeds: Good to Very Good")
