...
def generate_simple_report(results_file: str = "stress_test_dados/battle_results.json"):
    """Generate analysis report from battle results."""
    if not os.path.exists(results_file):
        print("âŒ No battle results found")
        return
    ...
    if weak_personalities:
        ...
        for pers in weak_personalities:
            print(f"   â€¢ {pers}")
        print("   â†’ Action: Increase approach_weight, combo_tendencia, or skill_agressividade")
        print("   â†’ Action: Reduce recuar_threshold to be more aggressive")
    
    if strong_personalities:
        print("\n2. STRONG PERSONALITIES (Win Rate > 65%):")
        for pers in strong_personalities:
            print(f"   â€¢ {pers}")
        print("   â†’ Action: Decrease combo_tendencia or skill_agressividade")
        print("   â†’ Action: Increase recuar_threshold to be more conservative")
    
    if imbalanced_matchups:
        print("\n3. IMBALANCED MATCHUPS:")
        for matchup in imbalanced_matchups[:3]:
            print(f"   â€¢ {matchup}")
        print("   â†’ Action: Add personality-aware decision logic")
        print("   â†’ Action: Adjust weapon effectiveness for specific pairs")
