...
def load_battle_results(results_file: str = "stress_test_dados/battle_results.json") -> List[Dict]:
    """Load battle results from JSON."""
    if not os.path.exists(results_file):
        print(f"âŒ File not found: {results_file}")
        return []
...
def load_characters(chars_file: str = "stress_test_dados/test_characters.json") -> Dict:
...
def identify_issues(results: List[Dict]) -> List[str]:
    ...
    for pers, rate in worst_pers:
        if rate < 0.40:  # Win rate below 40%
            issues.append(f"âš ï¸  Personality '{pers}' is underperforming ({rate*100:.1f}% win rate)")
    
    for pers, rate in best_pers:
        if rate > 0.60:  # Win rate above 60%
            issues.append(f"âš ï¸  Personality '{pers}' is overpowered ({rate*100:.1f}% win rate)")
    ...
    for cls, rate in worst_classes:
        if rate < 0.40:
            issues.append(f"âš ï¸  Class '{cls}' is underperforming ({rate*100:.1f}% win rate)")
    
    for cls, rate in best_classes:
        if rate > 0.60:
            issues.append(f"âš ï¸  Class '{cls}' is overpowered ({rate*100:.1f}% win rate)")
    ...
    for attr, stats in correlation.items():
        ...
        if abs(diff) < 0.1:
            issues.append(f"âš ï¸  Attribute '{attr}' has minimal impact on win rate (may need adjustment)")
    ...
    for (p1_pers, p2_pers), rate in sorted_matchups[:5]:
        ...
        if rate < 0.25 or rate > 0.75:
            issues.append(f"âš ï¸  Matchup '{p1_pers} vs {p2_pers}' is heavily imbalanced ({rate*100:.1f}%)")
    
    return issues


def generate_report(results_file: str = "stress_test_dados/battle_results.json") -> None:
    ...
    print(f"\nâœ“ Analyzing {len(results)} battle results...")
    ...
    if issues:
        ...
        print("\n  âœ“ No major imbalances detected!")
    ...
    if any("underperform" in issue.lower() for issue in issues):
        print("  â€¢ Adjust personality multipliers for underperforming archetypes")
        print("  â€¢ Consider increasing approach_weight or reducing retreat_weight")
    
    if any("overpowered" in issue.lower() for issue in issues):
        print("  â€¢ Fine-tune multipliers for overpowered personalities")
        print("  â€¢ Consider reducing combo_tendencia or skill_agressividade")
    
    if any("imbalanced" in issue.lower() for issue in issues):
        print("  â€¢ Review personality matchup-specific behavior")
        print("  â€¢ Consider adding personality-aware decision logic")
