...
class EmotionsMixin(_AIBrainMixinBase):
    """Mixin de emoções, humor, estados humanos, quirks, reações, ritmo e instintos."""
    ...
    def _atualizar_cooldowns(self, dt):
        ...
        # HIGH-04 fix: decai o timer do estilo temporário (style_switch instinto).
    ...
    def _atualizar_emocoes(self, dt, distancia, inimigo):
        ...
        # BUG-AI-05 fix: decai o modificador temporário de agressividade de volta a 0.
    ...
    def _tentar_pulo_evasivo(self, distancia, hp_pct):
        """Pulo evasivo de emergência.
        HIGH-03 fix: método era chamado em _processar_reacoes mas não existia
        em nenhum mixin — crash silencioso via AttributeError mascarado.
        Ativado quando HP está baixo e inimigo está próximo.
        """
        # Só pula em emergência real: HP < 35% E inimigo colado
        if hp_pct > 0.35 or distancia > 3.5:
            return False
        if self.cd_pulo > 0:
            return False
        # Traços que suprimem fuga
        if self.modo_berserk or "KAMIKAZE" in self.tracos:
            return False
        p = self.parent
        if p.z > 0:
            return False  # Já está no ar
        # Chance base — maior quanto mais desesperado
        ...
        if _r.random() < chance:
            ...
            self.cd_pulo = 1.2
            self.cd_reagir = 0.4
            _log.debug("[REAÇÃO] %s: pulo evasivo (hp=%.0f%%)", p.dados.nome, hp_pct * 100)
            return True
        return False


    def _tentar_dash_emergencia(self, distancia, hp_pct, inimigo):
        """Dash de emergência quando HP crítico e inimigo atacando.
        HIGH-03 fix: método era chamado em _processar_reacoes mas não existia
        em nenhum mixin — crash silencioso via AttributeError mascarado.
        Só usa uma skill de DASH disponível — sem criar dash do nada.
        """
        # Condições de emergência: HP muito baixo E ataque iminente
        if hp_pct > 0.25:
            return False
        ...
        if not dash_skills:
            return False
        import random as _r
        chance = 0.6 + (0.25 - hp_pct) * 2.0  # 60-110% → clampado
        ...
        if _r.random() < chance:
            ...
            if usou:
                ...
                _log.debug("[REAÇÃO] %s: dash emergência (hp=%.0f%%)", p.dados.nome, hp_pct * 100)
                return True
        return False
    ...
    def _tentar_contra_ataque(self, distancia, inimigo):
        ...
        # FP-01 fix: a condição anterior tinha lógica invertida.
        # `not inimigo.atacando` era sempre True fora do frame de ataque (inclusive durante
        # movimento, circulação, etc.), gerando falsos "vulnerável". A lógica correta exige
        # que o inimigo tenha ACABADO de atacar (cooldown > 0) E não esteja mais atacando.
        vulneravel = False
        acabou_de_atacar = (
    ...
    def _processar_instintos(self, dt, distancia, inimigo):
        """
        Processa instintos de combate — reações automáticas de alta prioridade.

        MEL-AI-01 (Sprint 4): todos os três triggers que estavam inativos foram corrigidos:
          • 'oponente_whiff'    — BUG-AI-02 fix (Sprint 1): janela_ataque["tipo"] = "whiff"
                                  agora é setado em _atualizar_leitura_oponente.
          • 'bloqueio_sucesso'  — BUG-AI-03 fix (Sprint 1): self.ultimo_bloqueio inicializado
                                  em __init__ e resetado em on_bloqueio_sucesso().
          • 'ataque_previsivel' — BUG-AI-04 fix (Sprint 1): leitura["padrao_detectado"]
                                  derivado de leitura["previsibilidade"] > AI_PREVISIBILIDADE_ALTA.
        O sistema de instintos agora está íntegro — todos os triggers são funcionais.
        """
    ...
    def _executar_instinto(self, acao, distancia, inimigo):
        ...
        if acao == "panic_dash":
            ...
            # BUG-AI-05 fix: usa modificador temporário — a personalidade base não é corrompida
            ...
            # Modo defensivo
            self.acao_atual = "RECUAR"
            # BUG-AI-05 fix: usa modificador temporário negativo
            ...
            # BUG-AI-05 fix: usa modificador temporário — decai automaticamente após a janela
            ...
            # Aumenta pressão
            # BUG-AI-05 fix: usa modificador temporário
            ...
            # sem timer de expiração — um MAGO poderia virar "AGGRO" para sempre.
            # Agora usa _estilo_override temporário (3-6s) sem corromper a personalidade base.
    ...
    def get_agressividade_efetiva(self):
        """Retorna agressividade com modificadores de ritmo e situacionais temporários.
        BUG-AI-05 fix: modificadores situacionais vão em _agressividade_temp_mod (não em agressividade_base),
        garantindo que a personalidade original não seja corrompida ao longo da luta.
        """
    ...
    def _rand(self):
        """Retorna um float [0,1) do pool pré-gerado por frame.
        ...
        em random.random() como segurança — isso nunca deveria acontecer em fluxo normal.
        """
        ...
        if pool:
            ...
            if idx < len(pool):
                ...
                return pool[idx]
        return random.random()  # fallback de segurança
