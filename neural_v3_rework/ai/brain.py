...
class AIBrain(PersonalityMixin, PerceptionMixin, EvasionMixin, CombatMixin, SkillsMixin, SpatialMixin, EmotionsMixin, ChoreographyMixin):
    ...
    def __init__(self, parent):
        ...
        self._tempo_sem_ataque_efetivo = 0.0  # Stall detection: tempo em ação ofensiva sem p.atacando
        ...
        # BUG-C2 fix: alvo atual — usado por _score_alvo para priorizar inimigos
        # que estão atacando aliados e pelo SUPPORT em _score_alvo.
    ...
    def _score_alvo(self, info_inimigo):
        ...
        if getattr(f, 'brain', None):
            brain = f.brain
            # BUG-C2 fix: _alvo_atual agora é sempre atribuído em processar().
            # Verifica se o inimigo está focando um aliado nosso — bônus de score
            # para incentivar interceptação e proteção de aliados.
        ...
        if team_role == "FLANKER":
            ...
            # MED-3 fix: f.alvo não existe em Lutador — substituído por
            # f.brain._alvo_atual que é atribuído em processar() (BUG-C2 fix).
    ...
    def _aplicar_team_orders(self, dt, distancia, inimigo, todos_lutadores):
        ...
        role_agg_mod = {
            ...
            "ARTILLERY":  -0.10,   # Mantém distância
            "SUPPORT":    -0.15,   # Mais defensivo
            "CONTROLLER": -0.05,   # Calculado, espera momento certo
        }
        # BUG-C3 fix: substituído acumulador (+= mod * dt) por lerp.
        # O acumulador nunca atingia equilíbrio porque ganho > decaimento em
        # brain_emotions (ex.: ARTILLERY acumulava -0.1/s mas só decaía +0.03/s,
        # atingindo o piso em ~6s e ficando lá para sempre).
        # Com lerp, o modificador converge suavemente ao setpoint do role
        # sem ultrapassá-lo, independente do dt.
        target_mod = role_agg_mod.get(role, 0.0)
        lerp_rate = min(1.0, 1.5 * dt)  # ~1.5 unidades/s de convergência
