...
class PerceptionMixin(_AIBrainMixinBase):
    ...
    def _calcular_estrategia_armas(self, distancia, inimigo):
        ...
        # CRIT-03 fix: antes, este método sobrescrevia p.alcance_ideal diretamente a
        ...
        # revertido pelo matchup de armas, lutando na distância errada.
        #
        # Solução: armazena o alcance tático como um OFFSET no percepcao_arma,
        # sem tocar p.alcance_ideal diretamente.  O offset é somado em
        # _calcular_alcance_efetivo() apenas para decisão de posicionamento.
        perc["alcance_tatico_offset"] = 0.0  # reset

        if perc["matchup_favoravel"] > 0.3:
            # Matchup favorável — fica na distância ideal da própria arma (sem offset)
    ...
    def _aplicar_modificadores_armas(self, distancia, inimigo):
        ...
        if tipo_ini == "Corrente":
            ...
            if arma_ini_estilo == "Mangual":
                ...
                # CRIT-01 fix: self.lutador não existe — atributo correto é self.parent
