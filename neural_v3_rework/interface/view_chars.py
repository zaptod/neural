...
from modelos import Personagem, LISTA_CLASSES, CLASSES_DATA, get_class_data
...
class TelaPersonagens(tk.Frame):
    ...
    def _criar_resumo_kit_magico(self, stat_count):
        nomes_skills = self._coletar_nomes_skills_preview()
        if not nomes_skills:
            return

        classes = [get_skill_classification(nome) for nome in nomes_skills]
        funcoes = []
        forcas = []
        elementos = []

        for classe in classes:
            if classe.classe_utilidade.value not in funcoes:
                funcoes.append(classe.classe_utilidade.value)
            if classe.classe_forca.value not in forcas:
                forcas.append(classe.classe_forca.value)
            if classe.elemento not in {"", "NEUTRO"} and classe.elemento not in elementos:
                elementos.append(classe.elemento)

        frame_kit = tk.Frame(self.frame_stats, bg=COR_BG_SECUNDARIO, bd=1, relief="flat")
        frame_kit.grid(
            row=(stat_count + 1) // 2 + 1,
            column=0,
            columnspan=2,
            padx=10,
            pady=(8, 4),
        ...
        tk.Label(
            frame_kit, text="Kit Magico",
            font=("Arial", 9, "bold"), bg=COR_BG_SECUNDARIO, fg=COR_ACCENT
        ).pack(anchor="w", padx=6, pady=(4, 2))

        linhas = [
            f"Skills: {len(nomes_skills)}",
            f"Forcas: {', '.join(forcas[:3])}",
            f"Funcoes: {', '.join(funcoes[:3])}",
            f"Elementos: {', '.join(elementos[:4]) if elementos else 'NEUTRO'}",
        ]
        for linha in linhas:
            tk.Label(
                frame_kit, text=linha,
    ...
    def _atualizar_info_personalidade(self):
        ...
        from ia.personalities import PERSONALIDADES_PRESETS
