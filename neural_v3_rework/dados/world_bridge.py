"""
NEURAL FIGHTS â€” WorldBridge
============================
Ponte de dados entre o sistema de luta/torneio e o World Map (pygame).

Quando uma luta termina, o vencedor conquista territÃ³rio para seu deus.
Os resultados sÃ£o gravados em world_map_pygame/dados/world_state.json e
world_map_pygame/dados/gods.json em tempo real.

Uso:
    from dados.world_bridge import WorldBridge
    WorldBridge.get().on_fight_result(winner_name, loser_name, duration, ko_type)

TambÃ©m expÃµe leitura do estado do mapa para a UI Tkinter:
    WorldBridge.get().get_god_standings()  â†’ list[dict]
    WorldBridge.get().get_territory_count(god_id) â†’ int
"""
...
# â”€â”€ GodEntry â€” objeto simples usado por view_chars.py â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
class GodEntry:
    """
    RepresentaÃ§Ã£o de um deus para a UI de criaÃ§Ã£o de personagens.
    Atributos sÃ£o mutÃ¡veis para que view_chars.py possa editar antes de salvar.
    """
...
# â”€â”€ B04: Resultado explÃ­cito de on_fight_result â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
@dataclass
class BridgeResult:
    """
    ...
    Atributos:
        ok      â€” True se a operaÃ§Ã£o foi executada (mesmo sem conquistar zona).
        zone_id â€” ID da zona conquistada, ou None se nenhuma disponÃ­vel.
        reason  â€” DescriÃ§Ã£o legÃ­vel do resultado (para logs/debug).
...
_HERE    = os.path.dirname(os.path.abspath(__file__))         # dados/
...
# Verifica se o mÃ³dulo world map existe
...
class WorldBridge:
    ...
    def _init(self):
        ...
        if WORLDMAP_AVAILABLE:
            _log.info("World Map detectado â€” ponte ativa.")
        else:
            _log.warning("World Map nÃ£o encontrado em %s â€” bridge desativada.", _WORLDMAP_DATA)

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # EVENTO PRINCIPAL â€” chamado apÃ³s cada luta
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    def on_fight_result(
        ...
        Chamado por:
          - view_luta.py   apÃ³s sim.run() retornar
          - tournament_mode.py em record_match_result()
        """
        # B04: retornos explÃ­citos em vez de None para tudo
        if not WORLDMAP_AVAILABLE:
            return BridgeResult(ok=False, zone_id=None, reason="worldmap indisponÃ­vel")
        if not winner_name:
        ...
        with self._lock_io:
            try:
                ...
                if not winner_god_id:
                    _log.warning("%s nÃ£o tem god_id â€” sem conquista de territÃ³rio.", winner_name)
                ...
                # TambÃ©m notifica AppState para quem escuta
                self._notify_app_state(winner_god_id, conquered)

                if conquered:
                    _log.info("ðŸ´ %s (%s) conquistou '%s'!", winner_name, winner_god_id, conquered)
                    return BridgeResult(ok=True, zone_id=conquered, reason="territÃ³rio conquistado")
                else:
                    return BridgeResult(ok=True, zone_id=None, reason="sem zonas neutras disponÃ­veis")

            except Exception as e:
                _log.exception("Erro em on_fight_result: %s", e)
                return BridgeResult(ok=False, zone_id=None, reason=f"exceÃ§Ã£o: {e}")

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # LEITURA â€” para a UI Tkinter
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    def get_god_standings(self) -> list:
        """
        Retorna lista de deuses com contagem de territÃ³rios e seguidores,
        ordenada por territÃ³rios (decrescente).
    ...
    def get_all_gods(self) -> List[GodEntry]:
        ...
        Usado por view_chars.py para o passo de seleÃ§Ã£o de divindade.
        """
        if not WORLDMAP_AVAILABLE:
            return []
        gds = _load_json_safe(_wm_path("gods.json"), {"gods": []})
    ...
    def get_god(self, god_id: str) -> Optional[GodEntry]:
        """
        Retorna um GodEntry pelo god_id, ou None se nÃ£o encontrado.
    ...
    def create_god(self, god_name: str, nature: str, nature_element: str,
        ...
        Usado por view_chars.py no wizard de criaÃ§Ã£o de deus.
        """
        import re
    ...
    def save_all(self):
        """
        Persiste alteraÃ§Ãµes feitas em GodEntry de volta ao gods.json.
        view_chars.py chama este mÃ©todo apÃ³s editar follower_count e lore_description.

        Nota: como GodEntry Ã© uma cÃ³pia em memÃ³ria, este mÃ©todo precisa da lista
        completa. Por isso view_chars.py deve usar o padrÃ£o:
            god = sync.create_god(...)
            god.follower_count = N
            sync.update_god(god)   â† usar update_god ao invÃ©s de save_all
        save_all() Ã© mantido como no-op para nÃ£o quebrar cÃ³digo existente.
        """
        pass  # WorldBridge salva atomicamente em cada operaÃ§Ã£o â€” save_all Ã© no-op

    def update_god(self, entry: GodEntry):
        """
        Atualiza os dados de um GodEntry existente no gods.json.
        Use apÃ³s modificar atributos de um GodEntry retornado por create_god() ou get_god().
        """
        if not WORLDMAP_AVAILABLE:
            return
        gds = _load_json_safe(_wm_path("gods.json"), {"gods": []})
    ...
    def _claim_territory(self, god_id: str) -> Optional[str]:
        ...
        Prioriza zonas vizinhas a territÃ³rios jÃ¡ controlados.
        """
        ...
        if not neutral_zones:
            _log.warning("Sem zonas neutras disponÃ­veis.")
            return None

        # Tenta encontrar zona vizinha (adjacente ao domÃ­nio atual)
        target_zone = None
        if already_owned:
            try:
    ...
    def _ensure_god_in_worldmap(self, god_id: str):
        """
        Garante que o deus existe em world_map_pygame/dados/gods.json.
        Se nÃ£o existir, cria entrada a partir do gods.json do game.
        """
        ...
        if game_god:
            ...
            # Mapa de nature display â†’ nature_element
            nature_map = {
    ...
    def _log_world_event(self, event: dict):
        """Adiciona evento ao histÃ³rico no world_state.json."""
        ws = _load_json_safe(_wm_path("world_state.json"), {})
    ...
    def _notify_app_state(self, god_id: str, zone_id: Optional[str]):
        """Notifica AppState sobre a conquista de territÃ³rio."""
        try:
            if _NEURAL not in sys.path:
                sys.path.insert(0, _NEURAL)
            from dados.app_state import AppState
            ...
            _log.warning("Erro ao notificar AppState sobre territÃ³rio: %s", _e)
