...
headless (sem interface grÃ¡fica) com debug completo.

Uso:
    python test_headless_battle.py

Funcionalidades:
    - Gera armas aleatÃ³rias de todas as raridades
...
# Evita crash de console no Windows quando o arquivo ainda contém textos legados.
...
def gerar_personagem_aleatorio(nome: str = None, classe: str = None, 
    ...
    personagem = {
        ...
        "_arma_obj": arma,  # ReferÃªncia interna para a arma
    }
...
class HeadlessBattle:
    """Simulador de batalha sem interface grÃ¡fica"""
    ...
    def _criar_personagem(self, char_data: dict, arma, pos_x: float, pos_y: float):
        ...
        try:
            ...
            personagem = Personagem(
                ...
                classe=char_data.get("classe", "Guerreiro (ForÃ§a Bruta)"),
                personalidade=char_data.get("personalidade", "AleatÃ³rio"),
            )
    ...
    def iniciar(self) -> bool:
        ...
        try:
            ...
            self.erros.append(f"Erro na inicializaÃ§Ã£o: {e}")
            log(f"Erro na inicializaÃ§Ã£o: {e}", "ERROR")
            traceback.print_exc()
            return False
    ...
    def _atualizar_projeteis(self, dt: float):
        ...
        for proj in self.projeteis[:]:
            try:
                ...
                self.erros.append(f"Erro ao atualizar projÃ©til: {e}")
                if proj in self.projeteis:
                    self.projeteis.remove(proj)
    ...
    def _atualizar_beams(self, dt: float):
        ...
        for beam in self.beams[:]:
            try:
                ...
                # Verifica colisÃ£o com linha do beam
                ...
                for alvo in [self.p1, self.p2]:
                    ...
                    import math
                    # DistÃ¢ncia do ponto ao segmento
                    dx = beam.x2 - beam.x1
                    dy = beam.y2 - beam.y1
    ...
    def _atualizar_areas(self, dt: float):
        ...
        for area in self.areas[:]:
            try:
                ...
                self.erros.append(f"Erro ao atualizar Ã¡rea: {e}")
    ...
    def executar_frame(self, dt: float = 0.016) -> bool:
        """Executa um frame da simulaÃ§Ã£o"""
        try:
            self.frame += 1
            ...
            for lutador, inimigo in [(self.p1, self.p2), (self.p2, self.p1)]:
                ...
                # Atualiza o lutador (mÃ©todo Ã© update, nÃ£o atualizar)
                try:
    ...
    def executar(self) -> dict:
        ...
        log(f"\nEstatÃ­sticas:", "HEADER")
        log(f"  ProjÃ©teis criados: {self.stats['projeteis_criados']}", "DEBUG")
        log(f"  Beams criados: {self.stats['beams_criados']}", "DEBUG")
        log(f"  Ãreas criadas: {self.stats['areas_criadas']}", "DEBUG")
        ...
        if self.erros:
            ...
            for erro in self.erros[:10]:  # MÃ¡ximo 10 erros
                log(f"  - {erro}", "ERROR")
...
def executar_teste_rapido():
    """Executa um teste rÃ¡pido com 2 personagens aleatÃ³rios"""
    log_header("TESTE RÃPIDO - 1 BATALHA ALEATÃ“RIA")
    
    # Gera personagens e armas
    arma1 = gerar_arma_aleatoria(raridade="Ã‰pico")
    arma2 = gerar_arma_aleatoria(raridade="Ã‰pico")
...
def executar_teste_todas_classes():
    """Testa todas as classes contra um oponente padrÃ£o"""
    from modelos.constants import LISTA_CLASSES
    ...
    for classe in LISTA_CLASSES:
        ...
        char2 = gerar_personagem_aleatorio(nome="Oponente", classe="Guerreiro (ForÃ§a Bruta)", arma=arma2)
...
def executar_teste_todas_raridades():
    ...
    from modelos.constants import LISTA_RARIDADES
...
def executar_teste_stress(num_batalhas: int = 10):
    """Executa mÃºltiplas batalhas aleatÃ³rias"""
    ...
    if erros_unicos:
        log(f"\nErros Ãºnicos encontrados ({len(erros_unicos)}):", "ERROR")
