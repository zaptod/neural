...
def test_battle_plan():
    skill_names = [
        "FÃªnix", "Muralha de Gelo", "Armadilha ElÃ©trica",
    ...
    armadilha = sss.skills.get("Armadilha ElÃ©trica")
    assert armadilha is not None, "Armadilha ElÃ©trica not profiled!"
    armadilha_data = SKILL_DB.get("Armadilha ElÃ©trica", {})
    is_trigger = not armadilha_data.get("bloqueia_movimento", True)
    print(f"[OK] Armadilha ElÃ©trica: trigger={is_trigger}, purposes={[p.value for p in armadilha.propositos]}")
    if is_trigger:
        assert SkillPurpose.OPENER in armadilha.propositos, "Trigger trap should have OPENER purpose"
        print("[OK] Armadilha ElÃ©trica has OPENER purpose (trigger trap)")
...
def test_condicoes_ideais(sss):
    ...
    if fenix:
        ...
        result = sss._condicoes_ideais("FÃªnix", sit_full)
        ...
        result2 = sss._condicoes_ideais("FÃªnix", sit_ok)
...
def test_role_detection():
    ...
    skill_names = ["FÃªnix", "Portal do Vazio", "InvocaÃ§Ã£o: EspÃ­rito", "Bola de Fogo"]
    char = FakeChar(skill_names)
    ...
    skill_names2 = ["Muralha de Gelo", "Armadilha ElÃ©trica", "Armadilha IncendiÃ¡ria", "Bola de Fogo"]
