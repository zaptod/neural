...
from nucleo.skills import SKILL_DB, listar_skills_para_ui
from dados import carregar_personagens  # D01 Sprint 10
...
class SimuladorManual(Simulador):
    """
    Simulador com controle manual do jogador e menus de configuraÃ§Ã£o
    """
    ...
    def atualizar_skills_disponiveis(self):
        ...
        if hasattr(self.controlando, 'skills_classe'):
            for sk in self.controlando.skills_classe:
                # Extrai dados para exibiÃ§Ã£o no HUD
                if "data" in sk:
    ...
    def aplicar_filtro_skills(self):
        """Aplica filtros Ã  lista de skills"""
    ...
    def trocar_skill_slot(self, slot, nova_skill):
        """Troca uma skill em um slot especÃ­fico"""
        if slot < 0 or slot >= 5:
            return
    ...
    def trocar_personagem(self, dados_personagem):
        ...
        # Cria novo lutador
        arma = self.controlando.arma  # MantÃ©m a arma atual
    ...
    def processar_input_manual(self, dt):
        ...
        # Se menu estÃ¡ ativo, processa input do menu
        if self.menu_ativo:
        ...
        if pygame.K_e in self.keys_just_pressed:
            ...
            if self.modo_oponente == "IA":
                ...
                oponente.brain = None
                print("Oponente: DUMMY (nÃ£o ataca)")
            else:
                self.modo_oponente = "IA"
                # Recria a IA
    ...
    def processar_input_menu(self, dt):
        """Processa input quando um menu estÃ¡ ativo"""
    ...
    def processar_menu_skills(self):
        ...
        # Se nÃ£o estÃ¡ editando slot, seleciona qual slot editar
        if self.slot_skill_editando is None:
            ...
            # EstÃ¡ editando um slot - navega na lista de skills
            # UP/DOWN = Navegar
    ...
    def desenhar_menu_skills(self, surface):
        """Desenha o menu de seleÃ§Ã£o de skills"""
    ...
    def desenhar_menu_personagem(self, surface):
        """Desenha o menu de seleÃ§Ã£o de personagem"""
        overlay = pygame.Surface((LARGURA, ALTURA), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 200))
        surface.blit(overlay, (0, 0))
        ...
        if 0 <= self.menu_selecionado < len(self.todos_personagens):
            ...
            stats_detail += f"ForÃ§a:{getattr(pers,'forca',10)} Destreza:{getattr(pers,'destreza',10)} "
