...
class Tournament:
    ...
    def load_participants_from_database(self, max_participants: int = 64) -> int:
        ...
        try:
            ...
            if not personagens:
                print("âŒ Nenhum personagem encontrado no banco de dados!")
                return 0
            
            # personagens sÃ£o objetos Personagem, nÃ£o dicionÃ¡rios
            self.participants = [p.nome for p in personagens[:max_participants]]
            
            # Ajusta para potÃªncia de 2
            self._adjust_to_power_of_two()
            
            print(f"âœ… {len(self.participants)} participantes carregados")
            return len(self.participants)
            
        except Exception as e:
            print(f"âŒ Erro ao carregar participantes: {e}")
            return 0
    ...
    def _adjust_to_power_of_two(self):
        """Ajusta nÃºmero de participantes para potÃªncia de 2"""
        n = len(self.participants)
        if n < 2:
            return
        
        # Encontra prÃ³xima potÃªncia de 2
        power = 1
        while power < n:
            power *= 2
        
        # Se jÃ¡ Ã© potÃªncia de 2, ok
        if power == n:
            return
        
        # Precisa adicionar byes ou truncar
        if n > power // 2:
            # Mais prÃ³ximo de power - adiciona byes
            byes_needed = power - n
    ...
    def generate_bracket(self) -> bool:
        ...
        if len(self.participants) < 2:
            print("âŒ MÃ­nimo de 2 participantes necessÃ¡rio!")
            return False
        ...
        print(f"âœ… Bracket gerado: {len(self.participants)} participantes, {num_rounds} rodadas")
        return True
    ...
    def _process_byes(self):
        """Processa vitÃ³rias automÃ¡ticas (BYEs)"""
        if self.current_round >= len(self.bracket):
            return
    ...
    def record_match_result(self, winner_name: str, duration: float = 0.0, 
        ...
        if winner_name not in [match.fighter1_name, match.fighter2_name]:
            print(f"âŒ Vencedor '{winner_name}' nÃ£o estÃ¡ nesta luta!")
            return False
        ...
        if match_id:
            try:
                from dados.app_state import AppState as _AS
        ...
        # â”€â”€ WorldBridge: conquista de territÃ³rio pelo deus do vencedor â”€â”€â”€â”€â”€â”€â”€
        try:
    ...
    def _advance_winner(self, match: TournamentMatch):
        ...
        # Descobre qual luta da prÃ³xima rodada
    ...
    def _advance_to_next_match(self):
        """AvanÃ§a para a prÃ³xima luta nÃ£o completada"""
    ...
    def get_bracket_display(self) -> str:
        """Retorna uma representaÃ§Ã£o visual do bracket"""
        lines = []
        lines.append("=" * 70)
        lines.append(f"  ðŸ† {self.name}")
        lines.append("=" * 70)
        ...
        for round_obj in self.bracket:
            ...
            for match in round_obj.matches:
                status = "âœ…" if match.completed else "â³"
                f1 = match.fighter1_name[:20]
                f2 = match.fighter2_name[:20]
                
                if match.completed:
                    winner = "â†" if match.winner_name == match.fighter1_name else "â†’"
        ...
        if self.champion:
            lines.append("\n" + "=" * 70)
            lines.append(f"  ðŸ† CAMPEÃƒO: {self.champion}")
            lines.append("=" * 70)
    ...
    def save_state(self, filename: str = "tournament_state.json"):
        ...
        print(f"âœ… Estado do torneio salvo via AppState")
    ...
    def load_state(self, filename: str = "tournament_state.json") -> bool:
        ...
        try:
            ...
            print("âœ… Estado do torneio carregado via AppState")
            return True

        except Exception as e:
            print(f"âŒ Erro ao carregar estado: {e}")
            return False
...
class TournamentRunner:
    ...
    def setup_match_config(self, fighter1_name: str, fighter2_name: str, cenario: str = "Arena"):
        """Configura o match_config para a prÃ³xima luta via AppState"""
        config = {
    ...
    def run_single_match_visual(self, match: TournamentMatch) -> bool:
        """Configura e lanÃ§a uma luta visual"""
        # Configura o match
    ...
    def run_single_match(self, match: TournamentMatch) -> Dict:
        """Executa uma Ãºnica luta usando simulaÃ§Ã£o simplificada"""
        from dados.app_state import AppState as _AS
        from modelos.constants import CLASSES_DATA
        
        # Carrega dados dos lutadores via AppState (single source of truth)
        state = _AS.get()
        personagens = state.characters
        armas = state.weapons
        ...
        if not p1_data or not p2_data:
            print(f"âŒ Personagens nÃ£o encontrados: {match.fighter1_name}, {match.fighter2_name}")
            return {"success": False, "error": "Personagens nÃ£o encontrados"}
        
        # SimulaÃ§Ã£o simplificada baseada em atributos
        ...
        def calcular_poder(personagem, arma_data):
            ...
            if arma_data:
                ...
                raridade_bonus = {
                    ...
                    "Ã‰pico": 1.4, "LendÃ¡rio": 1.6, "MÃ­tico": 2.0
                }
            ...
            # Poder de vida/resistÃªncia
            poder_defesa = mod_vida * 20
            
            # Poder de velocidade (chance de esquiva/crÃ­tico)
            ...
            poder_total *= random.uniform(0.8, 1.2)  # VariaÃ§Ã£o de Â±20%
        ...
        # Determina tipo de vitÃ³ria baseado na margem
        if margin > 50:
    ...
    def run_tournament_automated(self, delay_between_fights: float = 1.0):
        ...
        if not self.tournament.start_tournament():
            print("âŒ Falha ao iniciar torneio")
            return
        
        print(self.tournament.get_bracket_display())
        print("\n" + "=" * 70)
        print("  ðŸŽ® INICIANDO TORNEIO AUTOMÃTICO")
        print("=" * 70)
        ...
        while self.tournament.state != TournamentState.FINISHED:
            ...
            print(f"\nâš”ï¸  LUTA: {match.fighter1_name} vs {match.fighter2_name}")
            ...
            if result["success"]:
                ...
                print(f"   ðŸ† Vencedor: {winner} ({result['ko_type']} em {result['duration']:.1f}s)")
            else:
                # Decide aleatoriamente em caso de erro
                winner = random.choice([match.fighter1_name, match.fighter2_name])
                self.tournament.record_match_result(winner_name=winner, ko_type="DecisÃ£o")
                print(f"   ðŸ† Vencedor (decisÃ£o): {winner}")
...
if __name__ == "__main__":
    ...
    if match:
        print(f"\nðŸŽ® PrÃ³xima luta: {match.fighter1_name} vs {match.fighter2_name}")
